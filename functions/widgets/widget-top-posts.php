...
class video_wp_topposts extends WP_Widget {
     function video_wp_topposts() {
      $widget_ops = array('description' => esc_html__('Popular Posts - Display your Popular / Top Posts by comments.', 'video_wp'));
      parent::__construct(false, $name = ''. esc_html__('Custom: Popular Posts', 'video_wp') .'',$widget_ops); 
    }
...
<?php while ($antop->have_posts()) : $antop->the_post(); ?> 
...
<?php  } } 
add_action('widgets_init', create_function('', 'return register_widget("video_wp_topposts");')); // register widget
?>