...
class video_wp_module4 extends WP_Widget {
     function video_wp_module4() {
      $widget_ops = array('description' => esc_html__('== M4 == Display 2 Articles by category in one row.', 'video_wp'));
        parent::__construct(false, $name = '<i>'. esc_html__('Module 4: Articles by Categories', 'video_wp') .'</i>',$widget_ops);  
    }
...
?>
                ...
                <div class="article-btn"><?php if ( $title ) echo $before_title . esc_attr($title) . $after_title; ?></div><a class="more-cats" href="<?php echo esc_url( $category_link ); ?>"><?php esc_html_e( 'more', 'video_wp' ); ?> <i class="fa fa-caret-right" aria-hidden="true"></i></a>
                <div class="clear"></div>
                ...
                <div class="clear"></div>
                <div class="line_bottom_col" style="margin-top: 10px;"></div>
                <!-- end .modern-articles --> 
...
<?php
    ...
    $instance['number']    = is_numeric( $new_instance['number'] ) ? intval( $new_instance['number'] ) : 4;
    $instance['category']  = wp_strip_all_tags( $new_instance['category'] );
    return $instance;
    }

  function form( $instance ) {
    $defaults  = array( 'title' => '', 'category' => '', 'number' => 4 );
...
<?php  } } 
add_action('widgets_init', create_function('', 'return register_widget("video_wp_module4");')); // register widget
?>